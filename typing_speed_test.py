...
def start_timer():
    ...
    entry.focus()  # take cursor at text area
    entry.config(state=NORMAL)  # activating the text area
    startButton.config(state=DISABLED)  # disable start button while time is running

    for time in range(1,timelimit+1):
        elapsedtime = time
        # updating the elapsed timer label as time goes on
        elapsed_timer_label.config(text=elapsedtime)

        updateRemainingTime = remainingtime-elapsedtime
        # updating the remaining timer label
        remaining_timer_label.config(text=updateRemainingTime)

        # sleeping the for loop for 1 second
        sleep(1)
        # updating both the above labels
        root.update()

    # disable the text area so nobody can write after time limit is over
    entry.config(state=DISABLED)
    # activate the reset button
    resetButton.config(state=NORMAL)


# to keep count of total words typed, wrong words typed, accuracy, correct words typed per minute
def count():
    global wrongwords,elapsedtime,elapsedtimeInMinutes
    # extracting the displayed paragraph text in a list
    # this is how you extract text from a Label
    para_words = label_paragraph['text'].split()

    while elapsedtime != timelimit:
        # extracting the text inputted by the user
        # this is how you extract text from Text
        entered_paragraph = entry.get(1.0,END).split()
        totalwords = len(entered_paragraph)

    # comparing the given text with entered text to see how much was right or wrong
    ...
    # calculating correct words typed per minute
    wpm = (totalwords-wrongwords)/elapsedtimeInMinutes
    # update the correct words per minute timer label
    ...
    # updating the total words count label
    total_words_count_label.config(text=totalwords)

    # updating the wrong words count label
    wrong_words_count_label.config(text=wrongwords)


# starts both "start_timer()" & "count()" at the same time using two threads (both run concurrently)
# this is the command associated with start button in GUI
def start():
    t1 = threading.Thread(target=start_timer)
    t1.setDaemon(True) # when exit button is pressed, this will kill the two threads
    t1.start()
...
# command associated with the reset button
def reset():
...
titleLabel = Label(titleframe,text="Master Typing",font=('algerian',28,'bold'),bg='goldenrod3',fg='white',bd=10,width=38,relief=FLAT)
titleLabel.grid(row=0,column=0,pady=5)

# frame test will hold the label paragraph
...
# text frame will hold the text area where user will input text
text_frame = Frame(mainframe,bg='snow',relief='flat')
text_frame.grid(row=2,column=0)

# text area where user will input text
...
########################################################################################################################
# frame_output will contain labels "time remaining", "time elapsed", etc
...
########################################################################################################################
# frame containing start, reset & exit buttons
...
########################################################################################################################
# KeyboardFrame will contain the keyboard buttons

keyboardFrame = Frame(mainframe,bg='snow')
keyboardFrame.grid(row=4,pady=10)

########################################################################################################################
# frame for labels from 1 to 0 (note we're using labels for keyboard keys, not buttons)
...
########################################################################################################################
# keyboardFrame ends here ##############################################################################################
########################################################################################################################

# to change the color of a key to blue for a 100 ms when a key is pressed
def changeBG(widget):
    bg = 'black'
    widget.configure(background='blue')
    # after a time gap what should be done
    widget.after(100,lambda color=bg: widget.configure(background=color))


# binding the keyboard keys with their labels
